...
object Basics {
   ...
   */
  def lcm(a: Int, b: Int): Either[Int, String] = {
    if (a == 0 || b == 0) {
      return Right(s"Cannot calculate lcm for $a and $b.")
    }
    if (!(BigInt(a) * BigInt(b)).abs.isValidInt) {
      return Right("Result not predictable. Function doesn't work with BigInt.")
    }
    ...
    Left(lcmCalculator(minValue, maxValue, maxValue))
  }
